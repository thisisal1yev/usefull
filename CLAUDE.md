...
- **Mini App auth**: every `/api/*` endpoint sits behind `TelegramAuthGuard` — the webapp sends raw Telegram `initData` in the `x-telegram-init-data` header; `src/auth/init-data.ts` verifies the HMAC against `BOT_TOKEN` (max age 24 h) and the guard upserts the user onto `req.user`. API modules: `exam-questions` (bank; POST teacher/admin), `community` (Q&A) + `moderation` (reports), `matches` (partner matching), `lessons` (teachers/slots/bookings/admin + reminder & booking cron), `billing` (Stars invoices + expiry cron), `coach` (Gold coach assignment), `referrals` (invite → Premium days), `profile` (streak/progress/history), `users/me.controller`.
...
- **Schema**: `supabase/migrations/*.sql`, applied via Supabase MCP `apply_migration` (project id `ughjxpljxfqffzhbrozx`). Notable invariants: `users.tg_id` unique (Telegram identity), unique index on `bookings.slot_id` (double-booking guard), `unique (from_user, to_user)` on match_requests, `subscriptions.stars_tx_id` unique (idempotent payments), `referrals.invited_id` unique (each invite counts once).
- **Webapp**: four-tab Mini App (speaking / bank / invite / profile + conditional admin) with lucide-react icons. **Committed dark brand identity** (founder direction), not the Telegram theme — `webapp/src/styles.css` fixes the `tg-*` tokens to warm-dark + terracotta via `@theme`; edit that one file to repaint everything. Deployed on Vercel (https://usefull-fawn.vercel.app); the server is meant to run on a VPS via `deploy/` (Docker Compose + Caddy auto-HTTPS), and the Mini App's data screens stay on "loading" until `VITE_API_URL` points at a reachable server.
